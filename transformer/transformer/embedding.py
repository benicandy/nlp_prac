...
class AddPositionalEncoding(tf.keras.layers.Layer):
    ...
    def call(self, inputs: tf.Tensor) -> tf.Tensor:
        ...
        """
        tf.expand_dims(input: Tensor, axis: int): input の次元を axis 方向に一つ増やす
        tf.tile(input: Tensor, multiples: Tensor[int32, int64]): input を multiples
            で与えられた一次元ベクトル分敷き詰めた Tensor を返す
        ex. 
        [height, width, channels] -> tfexpand_dims(images, 0) -> [1, height, width, channels]
          -> tf.tile(images, [batch_size, 1, 1, 1]) -> [batch_size, height, width, channels]
        """
        ...
        positional_encoding = tf.sin(pos_matrix / depth_matrix + phase_matrix)  # [batch_size, max_length, depth]
...
class TokenEmbedding(tf.keras.layers.Layer):
    ...
    def call(self, input: tf.Tensor) -> tf.Tensor:
        ...
        embedding *= tf.expand_dims(mask, -1)  # 元々 PAD だった部分を 0 にする
